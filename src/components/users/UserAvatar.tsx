...
const getAvatarClasses = (shape = "circle", size = "md") => {
    ...
    const sizes = { 
        xs: "w-10 h-10", sm: "w-16 h-16", md: "w-24 h-24", lg: "w-32 h-32" };
    return `${shapes[shape]} ${sizes[size]}`;
};

const UserAvatar: React.FC<UserAvatarProps> = ({ shape = "circle", size = "md", user }) => (
    <div 
        className={`flex items-center justify-center cursor-pointer ${getAvatarClasses(shape, size)} hover:outline hover:outline-2 hover:outline-neon-primary`}
