...
export default function SocialAction() {
  const {
    ...
    isLoading,
    refetch,
  } = useQuery("admin/social-info/refer_social");
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentSocialLink, setCurrentSocialLink] = useState<SocialLink | null>(
    null,
  );
  const [upload, { isLoading: isMutating }] = useMutate({
    callback: undefined,
  });
  ...
  const handleSaveSocialLink = async (socialLink: SocialLink) => {
    const url = currentSocialLink
      ? `admin/social-info/update/${socialLink.id}`
      : "admin/social-info";
  ...
  const handleOnDelete = async (socialLink: SocialLink) => {
    const response = await upload(
      `admin/social-info/${socialLink.id}`,
      socialLink,
    );
    if (response?.success == true) {
      refetch!();
